...
Read a router configuration file from the specified local file. (This
method is not fully functional yet.)
...
then exit. There is an extra test in the file <xt/connecting-test.t>
which actually tries to connect to the router and do a few things. You
can either edit this file to insert the password and IP address of
your router, or you can set the following environment variables:
...
=item L<http://jp.yamaha.com/products/network/routers/> 

Yamaha's router site.

=item L<http://www.lemoda.net/networking/rtx1000/> 
...
L<Net::MAC::Vendor> - look up the vendor for a MAC.
